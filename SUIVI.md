...
Correction, 2 zones seulement.
- Nouvelle fonction top-level `hypeEtatSansImages(data)` (posee juste avant `HypeApp`) : retire toute chaine de plus de 4000 caracteres de `profil.avatar`, `profil.chevalPhoto` et `chevaux[].photo`. Les vraies URL Supabase (~80 caracteres) sont conservees. Appliquee SYSTEMATIQUEMENT, plus seulement en rattrapage.
- L'ecriture est differee de 400 ms et fusionnee (`window.__hypeEcritureEtat`), au lieu d'etre synchrone a chaque changement d'etat.

Verification. 15 blocs `<script>` inline valides par `node --check` (15/15 OK). Diff exhaustif des fonctions top-level dans les deux sens : 907 -> 908, la SEULE difference est `hypeEtatSansImages`, rien de perdu. Const/var top-level : 542 -> 542, identiques. `allerVersGalop` : exactement 3 occurrences. Aucun identifiant orphelin. Rendu Playwright reel compare a la base : `base.html` et le fichier corrige donnent tous deux EXACTEMENT 2 `pageerror` (`window.supabase` absent hors ligne, puis `TEINTES_HYPE`), texte rendu identique — aucune regression introduite.
...
MEDIAS LIVRES EN SESSION 92 (a pousser a la racine du depot) : `ouverture.mp4` (536x728, 10,1 s, 832 Ko, film d'intro de Linguae, appele ligne 3870 de lingo.html), `arrivee-labaule.mp4` (536x728, 5,1 s, 891 Ko), `carte-labaule.webp` (1086x1448, ratio 3/4 impose par `.pcarte`, 283 Ko), `fond-labaule.webp` (420x560, 1 884 o, flou 16 / saturation 0,72 / luminosite 0,52 cuits dans l'image, derive de la carte).

FILIGRANES : les exports CapCut portent DEUX filigranes superposes (« CapCut AI » + « CapCut ») en haut a gauche, x 23->160 / y 20->60 sur une source 560x752, effaces par `delogo=x=16:y=14:w=158:h=56`. Ils portent aussi des coins arrondis et un lisere noir de 2 px cuits : rogner 12 px sur chaque bord. `arrivee-newmarket.mp4` porte un filigrane « Ai » NON traite. `depart.mp4` est propre (480x794, deja vertical — contrairement a ce que disait le SUIVI). Verifier chaque fichier separement : les coordonnees changent d'un export a l'autre.

LINGUAE, ETAT REEL : 19 villes (`labaule` incluse, le SUIVI n'en listait que 18), donc **76 fichiers attendus** = 19 x (`arrivee-*.mp4`, `carte-*.webp`, `fond-*.webp`) + 10 fichiers uniques + 11 lexiques. Tout echec est SILENCIEUX sauf pour les videos d'arrivee, qui affichent le bloc « Emplacement de l'animation » (ligne 734) — c'est le temoin a chercher pour auditer les 19 villes. Les `fond-*.webp` se DERIVENT des cartes, aucune generation nouvelle necessaire.

TROU A TRANCHER : `hype-images-3.js`, `-4.js` et `-5.js` ne sont charges par personne (0 occurrence), alors que 1, 2 et 6 a 122 le sont. Soit fusionnes ailleurs et supprimables, soit une cle `kNNN` y vit encore et l'image est cassee en ligne.
...
## SESSION 131 · STORIES v16 — AJOUTER DES PHOTOS À SA STORY EN LIGNE

**Demande de Blandine (13/08, ~15 h 30)** : « modifier celle déjà en ligne et pouvoir lui rajouter quelques photos dessus. » Réalisé comme un geste depuis la story ouverte, pas comme une retouche de ligne existante (chaque photo reste une story — l'ajout publie des stories qui rejoignent la suite de son fil).

- **Menu ⋯, propriétaire seulement, en tête** : « ＋ Ajouter des photos » ouvre le sélecteur (multiple, plafond 10 partagé). Absent chez un visiteur.
- **Le composeur s'ouvre PRÉ-REMPLI** : le lieu et la musique de la story regardée (nouveaux props `lieuInitial`/`musiqueInitiale`, valeurs par défaut des états — rétrocompatible, aucun décalage de positions). La légende repart vide.
- **`dessus: true`** : la racine du composeur passe à 9450 quand il est ouvert depuis la visionneuse (9300) — il était à 9200, il aurait été invisible dessous. Depuis le bandeau : 9200 comme avant.
- **Après publication** : la visionneuse se ferme, le bandeau recharge (son `onFermer` appelle `charger()`) — la story rouverte contient les nouvelles photos à la suite. Annulation : retour à la story, minuteur relancé. Le glissé de fermeture est désarmé composeur ouvert (4e condition des 3 gardes).

**À l'écran : + « ＋ Ajouter des photos » en tête du menu ⋯ de tes propres stories.** Rien d'autre ne bouge.

### VÉRIFICATIONS
16/16 + module. **187 assertions** + 13 mentions. Nouvelles (9) : entrée présente chez le propriétaire, absente chez le visiteur, entrée de fichiers propre à la visionneuse, pré-remplissage vérifié (props → états), 9450 > 9300, fermeture-rechargement au publié, gardes du glissé étendues.

### Préparation Flutter (session 131)
Le composeur est devenu réutilisable en contexte (valeurs initiales par props, empilement par props) — exactement la forme d'un widget Flutter paramétré. Rien d'autre.
...
## SESSION 127 · STORIES v12 — LE FOND AU CHOIX DE L'AUTEUR, LE MENU ⋯

**Spec écrite de Blandine (13/08, ~04 h 30)** : photos horizontales jamais recadrées, deux fonds au choix de l'auteur (Noir = existant / Immersif = la même photo en cover floutée-assombrie derrière), sélecteur à la création ET à la modification, choix enregistré avec la story, **fond noir par défaut pour toutes les stories existantes**, ne rien casser. Puis deux ajouts oraux : « rassemble tout dans le menu avec les ⋯ » et (parenthèse interprétée, **déduction de Claude — lecture donnée à Blandine avant de coder, à confirmer à son réveil**) **la musique reste à part** sur la photo. Vérifié et dit : aucun bouton n'avait disparu, rien à « rétablir ».

### 1. LE FOND — CONTRAT DE DONNÉES
`hype_stories.fond` (SQL d'une ligne, fichier `fond-stories.sql`) : `"immersif"` ou `NULL` (= noir). `hsPublierStory(..., fond)` et `hsModifierStory(..., fond)` le portent ; les **replis colonne-manquante** l'abandonnent proprement (comme lieu/musique). `localMod` le propage : le changement est visible sans recharger.

### 2. LE SÉLECTEUR « Noir | Immersif »
Composeur : l'aperçu relève ses dimensions naturelles au chargement ; le sélecteur n'apparaît **que si la photo ne remplit pas le format vertical** (largeur ≥ hauteur), et le choix se voit **immédiatement** dans l'aperçu (calque flouté). Modifier : mêmes puces ; la feuille ne montrant jamais la photo, les dimensions sont relevées en la rechargeant (cache navigateur, coût nul). États déclarés APRÈS tous les existants (composeur 13-14, modifier 6-7) : les positions du harnais ne bougent pas.

### 3. LE FOND IMMERSIF EN VISIONNEUSE
Un calque derrière la photo : même image en cover, `blur(26px) brightness(0.5) saturate(1.05)`, débord de 24 px + `scale(1.12)` (pas de halo clair aux bords), `pointerEvents:none`, aucun zIndex — zones (1) et pastille (10) restent au-dessus. La photo d'origine reste **entière, en contain, jamais recadrée**. Conditions : choix de l'auteur + photo chargée + pas d'erreur + pas d'album.

### 4. LE MENU ⋯ (demande de Blandine)
Bouton ⋯ dans l'en-tête, avant la croix (absent des albums, qui n'ont pas d'actions). La rangée du panneau a **rejoint la feuille du menu** : ✦ Garder en souvenir, ✎ Modifier, Supprimer (propriétaire) / Signaler (visiteur) — mêmes handlers, mêmes permissions, rien de supprimé. Feuille : verre, poignée, ceinture tactile, tap dehors ferme ; le glissé de fermeture est désarmé menu ouvert (3 gardes). **La pastille ♪ reste à part sur la photo**, la croix reste en navigation.

**À l'écran : − la rangée Garder/Modifier/Supprimer (ou Signaler) du panneau · + le bouton ⋯ dans l'en-tête et sa feuille d'actions · + le sélecteur « Le fond : Noir | Immersif » au composeur et dans Modifier (photos non verticales seulement).** Les stories existantes ne changent pas d'un pixel.

### VÉRIFICATIONS
16/16 blocs + module. **155 assertions** + 13 mentions. Nouvelles (14) + 5 historiques adaptées au menu (menu fermé = panneau nu, menu ouvert = tout y est, fenêtre régex 600→760 pour la garde `menuOuvert`, signature 122 rendue inclusive, fixture musicale dédiée). Ancre ambiguë en cours de route (le démontage audio existe aussi dans la pastille de page) : refusée par le garde-fou, reprise via la signature unique d'`enregistrer()`.

### 🟠 NOTE D'ÉTAT — SUIVI DÉDOUBLONNÉ
La compaction de conversation (02 h 19) avait laissé DEUX sections 126 : celle écrite juste avant (complète, avec ses 3 assertions Lamotte au harnais) et une seconde, réécrite après coup faute de la voir. Le doublon est supprimé, **la version pré-compaction fait foi** — l'index de la session 126 était, lui, correct et unique (garde-fou d'unicité).

### 5. DEBORAH REJOINT LES AMBASSADEURS (demande de Blandine)
`deborah.guilmain@free.fr` ajoutée à `HYPE_AMBASSADEURS` (en minuscules — la comparaison passe par `toLowerCase().trim()`, la casse de saisie n'importe pas). Quatre ambassadeurs désormais : Liam, Evan, Mégane, Deborah — plus les modérateurs, inclus d'office. Aucun SQL : la liste vit dans l'index.

### Préparation Flutter (session 127)
Le fond est un **paramètre de présentation dans la donnée**, pas un embranchement de code : Flutter lira le même champ. Le menu ⋯ centralise les actions en une liste par permission — transposable tel quel.
...
**Correctif — une NOUVELLE adresse, sans toucher aux fichiers : suffixe de requête `?v=2`.**
Un premier plan (renommer les 12 fichiers en `-2`) a été remplacé quand Blandine a signalé ne plus savoir quelles images c'était : le suffixe `?v=2` obtient le même effet SANS re-téléverser quoi que ce soit. Pour le cache du téléphone, `baby-c3-brosse.jpg?v=2` est une adresse jamais vue → chargée fraîche puis mise en cache un an sous cette adresse ; pour Netlify, c'est le même fichier statique. Prochaine correction d'une de ces images : re-téléverser le fichier ET passer son suffixe à `?v=3`.
- **`hype-cours-baby.js` passe en version 112** : les 12 `src` gagnent `?v=2`, témoin `COURS_BABY_VERSION = "112"`. Preuve : après neutralisation (`?v=2` et `112` retirés) le fichier est IDENTIQUE à la v111 au caractère près. `node --check` OK. **Rien d'autre à pousser que ce fichier.**
...
### IMAGE DES QUIZ ENTIÈRE + MEMORY « ? » : ÉTAT EN FIN DE SOIRÉE

- **Image des quiz plus jamais coupée** (demande répétée de Blandine, enfin traitée) : le héros 37% passe de `cover` (recadrage) à **image entière en `contain` posée sur la même image floutée en fond** (blur 16px, léger zoom) — aucun bandeau vide, le sujet visible en entier. S'applique à TOUS les quiz (EcranQuiz, Galops et Baby).
- **Barème des quiz : NE PAS TOUCHER** (décision Blandine : les petits font les quiz avec un adulte). `nbEtoilesPourScore` reste 100%=3★/70%=2★.
...
### MEMORY « ? » — CAUSE TROUVÉE (k611-k614 jamais déclarées) + LAG + DIAGNOSTIC EMBARQUÉ

- **Cause des « ? » du Saut d'obstacles** : les paires utilisent k612/k614 (+k548/k551 servies par la boucle). **k611-k614 ne sont déclarées NULLE PART** — les boucles de convention couvrent k547-554 et k615-628, le trou 611-614 tombe entre les deux. Les photos ont été poussées « 10 fois » pour rien : sans déclaration de clé, la carte reste « ? », même en privé. **Correctif** : filet inséré après les lots (même bloc que k645) — `k611-614 = images/kNNN.jpg?v=1` si aucun lot ne les définit. ⚠️ Suppose que `images/k611.jpg` à `k614.jpg` existent en ligne — **test Blandine : ouvrir `/images/k612.jpg` dans Safari** ; si 404, me le dire (les fichiers portent peut-être d'autres noms).
- **Lag « injouable » pendant la partie** : l'animation `mpGlowPulse` (drop-shadow animé EN BOUCLE INFINIE sur la mascotte) est un tueur de perfs iOS connu — **retirée** (le pop et le flottement restent, `will-change: transform` ajouté). Si le lag persiste après ça, profiler la grille (re-rendus) en session dédiée.
- **Mode diagnostic embarqué** : ouvrir l'app avec **`?memdiag=1`** affiche en haut de la **liste des niveaux** du Memory (corrigé : d'abord accroché par erreur à l'écran Bienvenue, que les profils existants ne voient plus) la liste par niveau des paires SANS image (nom de la paire). Sert à inventorier les « ? » restants sans deviner — envoyer la capture, le mapping paire→clé est reconstituable depuis `hype-memory-poney.js`.
- Les autres niveaux incomplets (Robes, Copains, Sécurité, Friandises, Pansage) relèvent peut-être du même trou de déclaration OU d'images jamais créées — le memdiag le dira.

### MEMDIAG 22H30 — INVENTAIRE COMPLET : 4 PAIRES ORPHELINES DANS TOUT LE MEMORY

Capture de l'encadré memdiag (liste des niveaux) : **vie-poney : eau2 (k455), foin (k457) · allures : pas (k474) · obstacles : casque2 (k473)**. Tous les autres niveaux (Robes, Copains, Sécurité, Friandises, Pansage) : AUCUNE paire sans image — le filet k611-614 avait déjà tout réparé. Filet étendu aux 4 clés (`images/kNNN.jpg?v=1`, le lot gagne s'il existe). ⚠️ Reste à vérifier par Blandine que `images/k455.jpg`, `k457.jpg`, `k473.jpg`, `k474.jpg` existent en ligne (ouvrir chaque URL) — sinon téléverser ces 4 photos sous ces noms exacts, et le Memory est SOLDÉ.

### 22H39 — PLUS JAMAIS COINCÉ À N-1/N + CHANTIER « IMAGES TROP RESSEMBLANTES »

- **Bug constaté** (Saut d'obstacles, 5/6) : la paire sans image entre dans la grille avec deux cartes « ? » irrésolubles → partie infinissable. **Correctif structurel** : `MemoryPoneyGrille` filtre désormais les paires sans `src` AVANT le tirage — un niveau à l'image manquante se joue avec une paire de moins au lieu de bloquer. La couverture de niveau en entier (contain sur fond flou) est confirmée fonctionnelle sur la capture.
- **Remarque Blandine : « beaucoup d'images se ressemblent »** — exact sur Obstacles : envol/barre/réception/cavaletti = 4 scènes de saut quasi identiques en vignette, un enfant ne peut pas mémoriser des paires indistinguables. **Chantier contenu à préparer (prochaine session)** : redessiner les paires trop proches avec UNE action/objet distinctif par paire (Bible §16 : un seul geste par carte, test de lisibilité à 100 px ; la carte « réception » a même sa spec dédiée). Livraison recommandée en CHEMINS DIRECTS `images/memory-<niveau>-<paire>.jpg?v=1` (plus de clés k) — Claude fournira la liste des paires à refaire + prompts conformes à la Bible, Blandine génère, une seule passe d'intégration.
...
**Ancienne version (91) — Vidéos d'accueil et Communauté, bandeau messagerie, Lamotte et encart messages masqués, quête philosophie réparée, décalage horizontal corrigé — md5 `3e5dfd89a5eb0a8f4d6c03122d8bcdc7`, 10 506 977 octets. Part de `70a0c0d1` fourni par Blandine.**
...
### COMPLEMENT · LA QUETE SE VALIDE DEPUIS TOUS LES CHEMINS (demande Blandine)

Le premier correctif ne couvrait que la fiche du cheval. Blandine attendait l'inverse de ce que j'avais suppose : **la quete doit se valider quel que soit le chemin emprunte.**

**Il y a trois chemins dans l'app, et ils n'ecrivent pas au meme endroit :**
| chemin | ou la photo est ecrite | validait la quete avant ? |
|---|---|---|
| Fiche d'un cheval | `chevaux.photo_url` en base | non — l'etat global n'etait pas mis a jour (corrige plus haut) |
| Creation d'un cheval avec photo | `setChevaux` avec `c.photo` en local | **oui, deja** |
| Bouton « Mon cheval » de la page Cavalier | `profil.chevalPhoto` / `cheval_photo` en base | **non, et jamais** |

Le troisieme est le piege : `choisirCheval` ecrit un champ **du profil**, qui ne touche aucune ligne de la table `chevaux`. Aucune mise a jour d'etat n'aurait pu le faire valider — il fallait elargir la condition elle-meme.

`fait()` accepte donc desormais les deux formes : d'abord `profil.chevalPhoto` ou `profil.cheval_photo`, puis, a defaut, `photo_url` / `photo` sur l'une des lignes de `ctx.chevaux`. Le test du profil est enveloppe dans un `try`, et `ctx.profil` absent ne provoque rien.

**TEST DE LA VRAIE FONCTION**, extraite du fichier et executee en Node sur sept cas :
`rien` → non · `photo sur la fiche` → **valide** · `photo a la creation` → **valide** · `bouton Mon cheval` → **valide** · `champ base cheval_photo` → **valide** · `profil absent` → non · `tout vide` → non.

**Diff total de la session** : 43 lignes, 41 ajoutees, 2 retirees. Deux zones : `EcranCheval` (la repercussion dans l'etat global) et la definition de la quete dans `HYPE_QUETES_DECOUVERTE`.

⚠️ **A savoir sur ou cette quete s'affiche** : `HYPE_QUETES_DECOUVERTE` n'alimente que « La suite pour toi » et « Tes prochaines quetes » **sur l'accueil**, en lecture seule. L'ecran Quetes, lui, affiche les 14 familles de `HYPE_FAMILLES`, qui est un autre systeme — c'est lui qui verse l'XP. Le `xp: 15` inscrit sur la quete decouverte **n'est verse par personne** : c'est une valeur de maquette. A clarifier avec Blandine si elle attend un versement reel.

Ne pas confondre non plus avec la famille **« Fiches completes »**, qui compte les chevaux dont la fiche a photo **ET** robe **ET** age **ET** un parent renseigne (`hypeFicheComplete`). Une photo seule ne fait pas avancer cette famille, et c'est normal.
...
| 07/08 (100) | Claude (page Accueil) | **Quete `photo-cheval` corrigee et validee depuis les 3 chemins** (fiche, creation, bouton « Mon cheval » de la page Cavalier) : la photo etait ecrite en base et en local mais jamais dans l'etat global `ctx.chevaux` que lit la quete. On y repercute desormais le `photo_url` des que la base confirme. 26 lignes, confinees a `EcranCheval`. |
